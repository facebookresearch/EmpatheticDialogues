...
def get_parser():
    ...
    parser.add_argument("--embeddings-size", type=int, default=300, help='Transformer embedding size')
    parser.add_argument("--empchat-folder", type=str, help="Path to EmpatheticDialogues data folder")
    parser.add_argument("--hits-at-nb-cands", type=int, default=100, help='Num candidates to calculate precision out of')
    parser.add_argument("--learn-embeddings", action="store_true", help='Train on embeddings')
    ...
    parser.add_argument("--max-sent-len", type=int, default=100, help='Max num tokens per sentence')
    parser.add_argument("--rm-long-sent", action="store_true", help="Completely remove long Reddit sentences")
    parser.add_argument("--rm-long-contexts", action="store_true", help="Completely remove long Reddit contexts")
    parser.add_argument("--model", type=str, choices=["bert", "transformer"], help="Choice of retrieval model")
    ...
    parser.add_argument("--n-layers", type=int, default=6, help="Num Transformer layers")
    parser.add_argument("--normalize-emb", action="store_true", help="Normalize loaded embeddings")
    parser.add_argument("--normalize-sent-emb", action="store_true", help="Normalize context/candidate embeddings")
    parser.add_argument("--num-epochs", type=int, default=1000, help='Num epochs')
    parser.add_argument(
        "--optimizer", type=str, choices=["sgd", "adamax"], default="adamax", help='Choice of optimizer'
    )
    parser.add_argument("--pretrained", type=str, help='Path to pretrained model (will run eval only)')
    ...
    parser.add_argument("--stop-crit-num-epochs", type=int, default=-1, help='Num epochs to stop after if loss is not decreasing')
    parser.add_argument("--no-shuffle", action="store_true", help="Don't shuffle data during training")
    ...
    parser.add_argument("-bs", "--batch-size", type=int, default=32, help='Training/eval batch size')
